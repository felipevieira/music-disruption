...
def write_all_files(path):
    ...
    with open('../files/exp-3/all_files.txt', 'w') as my_file:
        for item in files:
            my_file.write("%s\n" % (item[0]))
...
def create_segmented_files(path, n_segments):
    with open(path, 'r') as list_of_files:
        ...
        for i in range((len(the_files) / n_segments)+1):
            with open('../files/exp-3/segmented/files-%i.txt' % (i+1), 'w') as segmented_file:
...
def extract_features_for_segments(segmented_features_path):
    for r, d, f in os.walk('../files/exp-3/segmented'):
        f.sort(key=natural_keys)
        for i in range(len(f)):
            file = f[i]
            subprocess.call(['python', '/home/felipev/workspace/transfer_learning_music/easy_feature_extraction.py', '/home/felipev/workspace/music-disruption/files/exp-3/segmented/files-%i.txt' % (i+1) , '/home/felipev/workspace/music-disruption/files/exp-3/segmented_features/features-%i.npy' % (i+1), '2'], cwd="/home/felipev/workspace/transfer_learning_music")
...
def combine_feature_files(feature_path):
    ...
    count = 0
    with open('../files/exp-3/all_features.txt', 'w') as outfile:
...
if __name__ == "__main__":
    # my_files = write_all_files('/home/felipev/datasets/forro em vinil resampled')    
    # create_segmented_files('../files/exp-3/all_files.txt', 100)

    # extract_features_for_segments('../files/exp-3/segmented_features/')

    combine_feature_files('/home/felipev/workspace/music-disruption/files/exp-3/segmented_features')
